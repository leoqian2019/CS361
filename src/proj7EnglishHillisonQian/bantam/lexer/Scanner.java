...
package proj7EnglishHillisonQian.bantam.lexer;

import proj7EnglishHillisonQian.bantam.util.CompilationException;
import proj7EnglishHillisonQian.bantam.util.Error;
import proj7EnglishHillisonQian.bantam.util.ErrorHandler;
...
public class Scanner
{
    ...
    /** LL as Queue to hold short list of pending characters **/
    private LinkedList<Character> pendingChars;
    ...
    public Token scan() {
        try{
            ...
            while (true) {
                // process chars in the stack first
                if (!pendingChars.isEmpty()) {
                    currentChar = (char) pendingChars.poll();
                } else {
    ...
    private Token scanWord() throws IOException {
        ...
        while(true){
            ...
            if(!Character.isLetter(next) && !Character.isDigit(next) && next != '_'){
                ...
                // check if the content match with any of the keyword
                if(content.equals("true") || content.equals("false")){
                    return new Token(Token.Kind.BOOLEAN, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("break")){
                    return new Token(Token.Kind.BREAK, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("cast")){
                    return new Token(Token.Kind.CAST, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("class")){
                    return new Token(Token.Kind.CLASS, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("var")){
                    return new Token(Token.Kind.VAR, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("else")){
                    return new Token(Token.Kind.ELSE, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("extends")){
                    return new Token(Token.Kind.EXTENDS, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("for")){
                    return new Token(Token.Kind.FOR, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("if")){
                    return new Token(Token.Kind.IF, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("instanceof")){
                    return new Token(Token.Kind.INSTANCEOF, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("new")){
                    return new Token(Token.Kind.NEW, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("return")){
                    return new Token(Token.Kind.RETURN, content,
                            sourceFile.getCurrentLineNumber());
                }
                if(content.equals("while")){
                    return new Token(Token.Kind.WHILE, content,
                            sourceFile.getCurrentLineNumber());
                }
                // if not, treat content as identifier
